...
def tensorize_triples(query_tokenizer, doc_tokenizer, queries, passages):
...
def tensorize_triples_(query_tokenizer, doc_tokenizer, queries, passages, scores, bsize, nway):
    # assert len(passages) == len(scores) == bsize * nway
    # assert bsize is None or len(queries) % bsize == 0
