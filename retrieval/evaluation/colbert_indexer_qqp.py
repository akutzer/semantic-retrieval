...
class ColBERTIndexerQQP(IndexerInterface):
    ...
    def index(self, queries: List[str], qids: List[str], bsize: int = 16) -> None:
        ...
        print('0000',len(batch_queries), len(batch_qids))
...
if __name__ == "__main__":
    ...
    indexer = ColBERTIndexerQQP(inference, device=DEVICE, dtype=torch.float16)

    queries = Queries(QUERIES_PATH)
    data = queries.values().tolist()
    qids = queries.keys().tolist()
    print(len(data),len(qids)) 
    ...
    print('1111',indexer.embeddings.shape)
    print('2222',indexer.iid2qid)
    print('3333',indexer.qid2iid)
    ...
    print('4444',indexer.embeddings.shape)
