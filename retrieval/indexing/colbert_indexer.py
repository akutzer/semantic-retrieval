...
class ColBERTIndexer(IndexerInterface):
    ...
    def index(self, passages: List[str], pids: List[str], bsize: int = 16) -> None:
        ...
        with torch.inference_mode():
            with torch.autocast(self.device.type):
                psgs_embedded = self.inference.doc_from_text(
                    batch_passages, bsize=bsize, show_progress=True
                )
            ...
            max_iids_per_pid = max(
                max(emb.shape[0] for emb in psgs_embedded), self.pid2iid.shape[-1]
            )

            # calculate the new height of the pid2iid matrix (aka the new maximal PIDs)
            max_pid = max(
                max(batch_pids),
                self.pid2iid.shape[0] + (self.offset if self.offset is not None else 0) - 1,
            )
            ...
            pid2iid = torch.full(
                (max_pid + 1 - new_offset, max_iids_per_pid),
                -1,
                dtype=torch.int32,
                device=self.device,
            )
            ...
            iid2pid = torch.empty(
                (self.iid2pid.shape[0] + num_new_iids,),
                dtype=torch.int32,
                device=self.device,
            )
            ...
            cat = torch.cat(
                [self.embeddings.cpu(), *map(lambda x: x.cpu(), psgs_embedded)], dim=0
            )
            del psgs_embedded
    ...
    def get_pid_embedding(
        self, batch_pids: List[torch.IntTensor]
    ) -> Tuple[List[torch.Tensor], List[torch.BoolTensor]]:
    ...
    def _new_passages(
        self, passages: List[str], pids: List[str]
        ...
        # else use only passages from unseen pids
            ...
            for pid, passage in zip(pids, passages):
                if (
                    pid > current_max_pid
                    or pid < current_min_pid
                    or self.pid2iid[pid - self.offset][0] == -1
                ):
