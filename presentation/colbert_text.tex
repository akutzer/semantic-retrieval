...
$$
\ell = -\frac{1}{B} \sum_{i=1}^{B} \log \frac{e^{S(q_i^+, d_i^+)}}{e^{S(q_i^+, d_i^+)} + \sum_{j=1}^{9}{e^{S(q_i^+, d_{i, (j)}^-)}}}
$$
For the FANDOM QA dataset, the loss function becomes:
$$
\ell_i = -\frac{1}{B} \sum_{i=1}^{B} \log \frac{e^{S(q_i^+, d_i^+)}}{e^{S(q_i^+, d_i^+)} + e^{S(q_i^-, d_i^+)}}
$$
